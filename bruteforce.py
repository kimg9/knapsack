...
if __name__ == "__main__":
    ...
    if args.csv:
        with open(args.csv, "r") as file:
            ...
            for row in reader:
                list_of_actions.append(
                    Action(row[0], int(row[1]), int(row[2].strip("%")))
                )
        ...
        print("The brute force algorithm bought:")

        for action in maximum_result:
            print(action.name)

        print(f"Total cost: {sum(action.price for action in maximum_result)}")
        print(f"Total return: {round(maximum_profit, 2)}")
